package com.docker.compose.security.jwt;
...
public class JwtHelper {
    ...
    static final String issuer = "My App";


    @Value("#{${accessTokenExpirationMinutes} * 60 * 1000}")
    private int accessTokeExpirationMs;
    @Value("#{${refreshTokenExpirationMinutes} * 24 * 60 * 60 * 1000}")
    private int refreshTokeExpirationMs;
    ...
    private JwtHelper(@Value("${accessTokenSecret}") String accessTokenSecret,
    ...
    public String generateAccessToken(User user){
        return JWT.create()
                ...
                .withExpiresAt(new Date(new Date().getTime() + accessTokeExpirationMs))
    ...
    public String generateRefreshToken(User user, String tokenId){
        return JWT.create()
                ...
                .withIssuedAt(new Date())
                .withExpiresAt(new Date(new Date().getTime() + refreshTokeExpirationMs))
    ...
    private Optional<DecodedJWT> decodeAccessToken(String token){
        try {
            return Optional
                    .of(accessTokenVerifier.verify(token));
    ...
    private Optional<DecodedJWT> decodeRefreshToken(String token){
        try {
            return Optional
                    .of(refreshTokenVerifier.verify(token));
        } catch (JWTVerificationException e){
